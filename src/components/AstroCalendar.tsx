...
const ReadingGuideView = lazy(() => import("./ReadingGuideView").then(m => ({ default: m.ReadingGuideView })));
...
export const AstroCalendar = () => {
  ...
  const getTitle = () => {
    ...
    if (viewMode === "reading-guide") {
      return "Chart Walkthrough";
    }
    if (viewMode === "vedic") {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-5 py-10 md:px-10 md:py-16">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex flex-wrap gap-1 rounded-sm bg-secondary p-1">
              ...
              <button
                onClick={() => {
                  if (userNatalChart || savedCharts.length > 0) {
                    setViewMode("reading-guide");
                  } else {
                    setViewMode("charts");
                  }
                }}
                className={`flex items-center gap-1.5 rounded-sm px-3 py-2 text-[11px] uppercase tracking-widest transition-all ${
                  viewMode === "reading-guide"
                    ? "bg-primary text-primary-foreground"
                    : userNatalChart || savedCharts.length > 0
                      ? "text-muted-foreground hover:text-foreground"
                      : "text-muted-foreground/60 hover:text-muted-foreground"
                ...
                title={
                  userNatalChart || savedCharts.length > 0
                    ? "Chart Walkthrough — guided live reading"
                    : "Add a chart to use the Chart Walkthrough"
                }
              >
                🧭 Chart Walkthrough
              </button>
        ...
        {viewMode === "annual-tables" && <AnnualTablesView year={currentDate.getFullYear()} activeChart={activeChart} />}

        {viewMode === "guide" && (
          <GuideView
            userNatalChart={userNatalChart}
            savedCharts={savedCharts}
            onNavigateToView={(view) => setViewMode(view)}
          />
        )}
        ...
        {viewMode === "ask" && (
          <AskView
            ...
            savedCharts={savedCharts}
            selectedChartId={selectedChartForTiming}
            solarReturnCharts={solarReturnCharts}
          />
        )}

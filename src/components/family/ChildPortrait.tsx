                        ...
                        body: (
                          <>
                            ...
                            <Row label="The Physics, How These Three Links Actually Work">
                              In a standard chart, the Rising is the <strong>front door</strong> people see, the Chart Ruler is the <strong>person running the room</strong> behind that door, and the Dispositor is the <strong>final check</strong> the whole system reports up to. {N}'s door is <M>{ascStr}</M>, which is why the surface looks like classic Libra: read the room, smooth the edges, keep things pleasant. The part running the room is <M>{rulerStr}</M>{rulerIsAnyZero ? <>, turned all the way up in {cr.rulerSign}</> : null}, and {cr.rulerSign === "Sagittarius" ? "Sagittarius needs space, honesty, movement, and a way out" : `${cr.rulerSign} has its own strong need`}. {dispoStr ? <>That part checks in with <M>{dispoStr}</M>, which {dispo!.sign === "Taurus" ? "asks simple body questions: Will this leave me steady? Will I still have enough money, time, calm, and control over my own life?" : `asks what will make the choice feel solid`}. </> : null}{mutualReception ? <>But the chain does not end there. Because {dispo!.name} in {dispo!.sign} is itself ruled by {cr.rulerName}, the decision gets passed right back. There is no outside final boss. <M>{cr.rulerName}</M> and <M>{dispo!.name}</M> keep checking each other, so both needs have to be included. </> : null}
                            </Row>
                            {mutualReception ? (

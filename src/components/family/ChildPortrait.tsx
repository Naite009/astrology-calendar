                          ...
                          <>
                            ...
                            {hasVoltage ? (
                              <Row label="The Anchor, Why The Extreme Degrees Change The Reading">
                                ...
                                {dispoIsAnaretic ? <>The Dispositor sits at <M>{fmtDeg(dispo!.degree)} {dispo!.sign}</M>, the very last degree of the sign. That spot is called the <strong>anaretic degree</strong>, sometimes "the degree of fate." This matters because a normal {dispo!.name} in {dispo!.sign} placement would read as {dispo!.sign === "Taurus" ? <>"settled comfort, stay put, build slow security, eat well, do not move,"</> : <>a relaxed mid-sign version of {dispo!.sign}</>}. <strong>That reading does not apply here.</strong> At 29°, {dispo!.name} has already lived the whole sign and is one breath from leaving it, so the placement acts like a senior expert on their final day, urgently trying to ship the definitive version of {dispo!.sign === "Taurus" ? <>everything Taurus knows: money, body, food, real-world resource, lasting value</> : <>everything {dispo!.sign} knows</>}. So when this reading says "not background comfort, not staying put," it means <em>{dispo!.name} is not staying put</em>. The standard Taurus-as-couch reading is off the table. This is <strong>{cfoLabel}</strong>: high-pressure, urgent, "now-or-never" mastery, a felt mandate to build <strong>Leveraged Expertise</strong> ({dispo!.house === 8 ? <>worked out inside the <strong>8th House</strong>: shared money, other people's capital, deep specialist knowledge</> : dispo!.house ? <>worked out inside the <strong>{ord(dispo!.house)} House</strong></> : <>worked out in the dispositor's house</>}).</> : null}
                              </Row>
                            ) : null}
                            <Row label="The Physics, How These Three Links Actually Work">
                              In a standard chart, the Rising is the <strong>front door</strong> people see, the Chart Ruler is the <strong>operator</strong> behind that door, and the Dispositor is the <strong>final anchor</strong> the whole system reports up to. {N}'s door is <M>{ascStr}</M>, which is why the surface looks like classic Libra: read the room, smooth the edges, keep things pleasant. The operator behind that door is <M>{rulerStr}</M>{rulerIsAnyZero ? <>, running at the rawest amplitude {cr.rulerSign} can produce</> : null}, and {cr.rulerSign === "Sagittarius" ? "Sagittarius has one non-negotiable requirement: freedom to move" : `${cr.rulerSign} has its own non-negotiable requirement`}. {dispoStr ? <>That operator reports up to <M>{dispoStr}</M>, which {dispo!.sign === "Taurus" ? "is the part of the system that converts lived experience into durable expertise and real-world resource, the kind that compounds and can be drawn on later" : `is the part of the system that converts experience into resource`}. </> : null}{mutualReception ? <>But the chain does not end there. Because {dispo!.name} in {dispo!.sign} is itself ruled by {cr.rulerName}, the authority gets handed right back. There is no outside final boss. <M>{cr.rulerName}</M> and <M>{dispo!.name}</M> host each other and co-sign every decision in real time. </> : null}
                            </Row>
                            {mutualReception ? (

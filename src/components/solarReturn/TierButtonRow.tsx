...
const TIER_COLORS: Record<TierId, { bg: string; text: string; border: string }> = {
  t1: { bg: '#E1F5EE', text: '#085041', border: '#9FE1CB' },
  t2: { bg: '#EEEDFE', text: '#3C3489', border: '#CECBF6' },
      ...
      {TIER_BUTTONS.map(({ id, label }) => {
        ...
        return (
          <button
            ...
            className="px-4 py-1.5 rounded-full text-xs font-medium transition-all duration-150 cursor-pointer select-none hover:opacity-80"
            style={{

...
Tone: Wise counsel, not a horoscope. Do not overpromise. Differentiate chemistry from compatibility. Be direct about contradictions.`,
  ...
  },
  {
    ...
    prompt: (name, date, time, loc) =>
      `Using the full natal chart for ${name}, born ${date} at ${time} in ${loc}, provide a deep parent-focused reading. Today's date is ${today()}. The person asking IS the parent — this reading is about how THEY parent, what their specific child experiences of them, and how to grow as a parent for THIS child. Treat the relationship as directional: PARENT chart → CHILD chart. The parent's planets are the climate; the child's luminaries (Sun, Moon) and angles (Ascendant, MC) are the weather sensor. Direction matters absolutely — "Parent Mars → Child Moon" is a different reading than the reverse, and you must never invert it. If a CHILD CHART block is present in chart context, use it as the second chart for every cross-aspect calculation. If NO CHILD CHART block is present, follow the "NO CHILD CHART FALLBACK" rules below.

VOICE RULES (NON-NEGOTIABLE):
- Speak directly to the parent ("you", "your child"). If a child chart is provided, use the child's first name at least once per section.
...
ORB RULES (HARD): Use ≤6° for conjunctions/oppositions/squares, ≤4° for trines/sextiles, ≤2° for all other aspects (quincunx, semi-square, sesquiquadrate, etc.). When listing the "5 tightest cross-aspects," rank strictly by the actual measured orb in degrees — never by perceived importance.

AGE CALIBRATION: Compute the child's age as of today (${today()}) using the birth date in the CHILD CHART block. Calibrate language: under 6 = pre-verbal/sensory, 6–11 = developmental/identity-building, 12–17 = identity-formation/autonomy, 18+ = adult-child relational. If NO child chart is provided, write for developmental range 5–12 as the default and explicitly state this assumption at the top of Section 6.

RETROGRADE RULE (SELF-CONTAINED): In any planet list formatted as "{Chart} {Planet} in {Sign}", append " R" immediately after the planet name if that planet is retrograde. Example: "Parent Mercury R in Scorpio in the 12th."

CROSS-ASPECT CITATION: Every cross-aspect MUST cite both signs/degrees AND the orb (e.g., "your Mars at 14° Aries square your child's Moon at 15° Capricorn, orb 1°02'"). No vague "your Mars hits their Moon."

Use this EXACT section order:

SECTION 1: THE ESSENCE OF YOU AS A PARENT — Do NOT open with a list of placements (the placement_table handles that). Open with 2 synthesizing sentences of essence describing the climate this parent creates around any child. Then expand by referencing specific placements (Sun, Moon, Mars, Mercury, Saturn) as you go. No jargon.

SECTION 2: WHO YOUR CHILD ACTUALLY IS — If a child chart is provided: open by naming the child's Sun, Moon, Ascendant, Mercury, and Mars (sign + house), then synthesize the nervous system, attention style, and emotional rhythm of THIS child. Include the child's current age and developmental stage.
  NO CHILD CHART FALLBACK: Instead, describe the emotional weather a child living in this home will typically encounter, organized by three developmental phases — early childhood (0–6), middle childhood (6–12), and adolescence (12–18). Be concrete about what each phase will feel like under this parent's climate.

SECTION 3: HOW YOUR CHILD EXPERIENCES YOU (TOP 5) — List the 5 tightest cross-aspects between PARENT planets and CHILD Sun/Moon/Ascendant/Mercury/Mars, ranked strictly by orb (apply the ORB RULES above). For each: (a) what the child feels in their body in a real moment (homework, bedtime, transition, conflict, praise), (b) the cross-aspect with both signs/degrees and orb, (c) what the child has likely concluded about themselves from this pattern, (d) ONE concrete parenting move you can use this week. If no child chart, describe the 5 ways this parent's tightest natal aspects shape any child in proximity.

SECTION 4: WHERE YOUR DRIVE LANDS HARDEST ON THIS CHILD — Parent's Mars sign/house/aspects, AND the specific contact between parent's Mars and child's Moon/Ascendant/Mercury/Mars. What this specific child feels when you're frustrated even at something else. The repair move that resets THIS child's nervous system fastest.

SECTION 5: THE INHERITED PATTERN — First paragraph (2–3 sentences): synthesize the unconscious pattern from the parent's OWN Saturn, Chiron, Pluto, and Nodes (sign/house) — what was absorbed from the parent's own upbringing that now lives in them. Second paragraph: name each cross-contact from parent Saturn/Chiron/Pluto/Nodes to the child's Sun/Moon/Ascendant explicitly, ONE sentence per contact. This is where the chain either continues or breaks.

SECTION 6: HOW YOUR WORDS LAND — Parent's Mercury sign/house/aspects, AND the parent-Mercury → child-Mercury / child-Moon contact. The kind of sentence that calms THIS child versus the kind that rings for days. Two practical phrasing examples drawn from the child's Mercury sign. (If NO child chart: state the 5–12 default assumption at the top of this section, then give phrasing examples for that age range.)

SECTION 7: WHAT THIS CHILD NEEDS FROM YOU THAT ANOTHER CHILD WOULDN'T — Tied to the child's Moon (emotional fuel), Ascendant (how they need to be met at the door), Mercury (talking style), and Mars (how they need to be allowed to push). Compare and contrast: "Most parenting advice would say X, but for THIS child you need Y."

SECTION 8: THE ONE PRACTICE FOR THE NEXT 90 DAYS — Base the single practice on whichever has the most urgency in the next 90 days: a current/upcoming transit OR a cross-aspect activation. If no transits are salient, use the parent's hardest Mars or Saturn cross-contact to the child as the anchor. Direct, plain, kind, doable this week and sustainable for 90 days.

SECTION 9: WHERE YOU TWO CLICK — Tightest supportive cross-aspects (trine/sextile within 4°) between parent and child, framed as the shared language and the moments where parenting feels easy and reciprocal. Name 2–3 concrete activities or rhythms that play to these strengths.

STRUCTURAL OUTPUT: Begin with placement_table titled "Parent Key Placements". If a child chart is provided, follow with a second placement_table titled "Child Key Placements" (REQUIRED — pull from CHILD CHART block). Then narrative_section per SECTION above. End with summary_box titled "Parenting Strategy" with items: "Your Climate", "Who This Child Is" (or "The Climate Children Encounter" if no child chart), "What Your Child Feels Most", "The Pattern to Break", "This Season's Practice". The "question_type" in your JSON output MUST be exactly "parenting".`,
  },
];
          ...
          <div className="rounded-lg border border-border bg-card p-4 space-y-4">
            ...
            <div className="flex justify-end">
              <Button
                ...
                disabled={activeTopic.id === "parenting" && childChartOptions.length > 0 && !childChartId}
                className="text-xs"
              >

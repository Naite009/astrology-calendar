...
NATAL PLUTO — RETROGRADE STATUS VERIFY (NON-NEGOTIABLE — addresses the recurring "natal Pluto: claimed retrograde but truth is direct" defect):
Before writing ANY sentence that says or implies natal Pluto is retrograde (whether qualified as "natal Pluto", unqualified "Pluto", or via separated phrasing like "Pluto in the 8th, retrograde" / "Pluto … is currently retrograde"), check the natal placement table for Pluto's row. If the row does NOT carry an R / Rx / ℞ marker AND the row's retrograde field is not true, then natal Pluto is DIRECT in this chart and you MUST NOT describe it as retrograde. Do NOT use generation-level shorthand ("Pluto retrograde generation", "the Pluto Rx pattern in this person") unless the table confirms the planet is retrograde. The same verify-before-writing rule applies to every other slow-mover: Saturn, Uranus, Neptune, Chiron — natal retrograde claims must be backed by the natal placement table for THIS chart, not assumed from generational tendency.

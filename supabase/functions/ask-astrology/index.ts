...
// REMOVED (Rule 2 — No Sweeps): `factsAwareNatalHouseSweep` was a regex
// prose-mutation sweep that conflicted with `correctSrPlanetHousesInProse`
// (the SR-side sweep, also removed). The two would overwrite each other
// on the same sentence (e.g. natal Lilith H1→H8→H1). It is replaced by
// `runPlacementTableValidator` which compares every natal/SR house, sign,
// and retrograde claim against the placement tables and fails the
// generation loud on any mismatch. `parseNatalHousePlacements` (above) is
// retained because the validator uses it as natal truth source.
// Do not reintroduce this sweep. See
// .lovable/memory/architecture/ask-astrology/no-prose-sweeps-placement-table-truth.md
...
const runAccuracyReview = (parsedContent: any, chartContext: string) => {
  ...
  const SKIP = new Set([
    ...
    "_post_gate_safety","_final_hygiene","_sr_house_copy_warning","_source_call","_relationship_contract",
...
const SENTENCE_DEDUPE_SAFE_KEYS = new Set([
  ...
  "_post_gate_safety", "_final_hygiene", "_accuracy_review", "_verified_activations", "_source_call", "_relationship_contract",
  "type", "label", "name", "planet", "aspect", "natal_point", "symbol", "tag",
  "house", "sign", "degrees", "generated_date", "birth_info",
...
NATAL PORTRAIT PRIMARY-SOURCE RULE — APPLIES TO EVERY READING TYPE (relationship, career, health, money, spiritual, relocation, solar_return, natal, timing, general): You have access to pre-calculated Natal Portrait data injected into this prompt under the heading "--- NATAL PORTRAIT (PRE-CALCULATED — PRIMARY SOURCE OF TRUTH) ---". Use it as your primary source of truth for who this person is at the natal level. Do not contradict it. When the portrait data gives you a synthesis or interpretation (e.g., lifePurpose.coreSynthesis, lifePurpose.identityStatement, dominantPlanets.rankings[].role, lifetimeWisdom.lifeDirection, powerPortrait.driveSource, powerPortrait.burnoutPattern, patterns[].description, houseEmphasis[].description, relationshipBlueprint.summary, careerMoneyMap.summary, healthVitality.summary, spiritualKarmic.summary, etc.), expand it into plain conversational language with one concrete real-life example rather than repeating it verbatim. The natal portrait describes the permanent baseline. The Solar Return data (where present) describes what THIS specific year is doing to that baseline — always interpret SR placements as movement against the natal portrait, never in isolation. The placement tables remain authoritative for raw degrees, signs, houses, retrograde markers, and aspects; the portrait block is authoritative for synthesis and interpretation.
...
NO REFUSAL, NO INPUT META-REFERENCES — ABSOLUTE HARD RULE (overrides all other generative tendencies):
...

{
  ...
  "question_type": "natal" | "solar_return" | "relationship" | "relocation" | "career" | "health" | "money" | "spiritual" | "timing" | "general",
...
- DEGREE ACCURACY RULE — MANDATORY: Before writing any planet's degree in prose, verify it against the natal placement table provided in this call. Never use a degree from one planet's row for a different planet. If you reference Jupiter's degree, it must come from Jupiter's row. If you reference Neptune's degree, it must come from Neptune's row. This applies to every planet, every aspect orb calculation, and every conjunction/aspect statement. When stating an aspect orb (e.g., "conjunct X at Y° (orb approximately Z°)"), recompute the orb from the two correct degrees you just pulled from the placement table — never reuse an orb from a different aspect pair.
- PISCES RULERSHIP RULE: When writing about a Pisces house cusp ruler, always use BOTH traditional and modern rulers consistently: "traditional ruler Jupiter and modern ruler Neptune". Never switch between Jupiter-only and Neptune-only within the same section. Discuss both planets' placements (sign, house, aspects) when interpreting a Pisces-ruled house. The same dual-rulership approach applies to Scorpio cusps (Mars + Pluto) and Aquarius cusps (Saturn + Uranus) when both rulers materially shape the interpretation.
...
═══════════════════════════════════════════════════════════════════════════════
UNIVERSAL READING TYPE BASE — INHERITED BY EVERY READING TYPE (PRESENT AND FUTURE)
═══════════════════════════════════════════════════════════════════════════════
The rules in this block are the SHARED BASE that every reading type automatically inherits. They apply to EVERY question_type — those defined below (relationship, relocation, career, health, money, natal, solar_return, spiritual, timing, general) AND any new question_type added in the future. A new reading type does NOT need to redeclare any of these rules; they are inherited by default. A new reading type may add its own additional rules ON TOP, but it can NEVER opt out of these. If a per-type rule below appears to conflict with the base, the base wins unless the per-type rule explicitly says "OVERRIDES UNIVERSAL BASE" with a justification.

BASE RULE 1 — TIMING TRANSIT PERSONALIZATION (applies to every transit interpretation, in every reading type, in every section that names a transit — timing_section, narrative_section inline references, summary_box "When" / "Best Windows" / "Caution Windows" callouts):
  (a) NAME THE SPECIFIC NATAL POINT BEING HIT, with sign and (when available) house. Write "Pluto squaring your natal Mars in Scorpio in your 5th house" — NOT "Pluto squaring this part of your chart", NOT "Pluto squaring this part of your natal chart", NOT "Pluto squaring this part of your natal makeup", NOT "Pluto square your X" without a sign, NOT "the Pluto square activating you", NOT "this transit hitting your natal placements". The natal point's planet + sign (and house if available) MUST be named explicitly inside the interpretation sentence itself.
  (b) GIVE ONE CONCRETE REAL-LIFE SCENARIO specific to what THAT natal point means for THIS person — a situation that could plausibly happen in a week or a month. Do NOT write a generic textbook description (e.g., "brings intensity and power dynamics to the surface"). The scenario must be grounded in what the natal point actually rules in this chart. The concrete scenario MUST be a recognizable scene from daily life (a work conversation, a money decision, a body sensation, a relationship moment, a calendar choice) — not a restatement of the planet's symbolic meaning.
  (c) NEVER end with a generic horoscope closer. The phrases "awareness is the work", "the lesson is to stay present", "this is an opportunity for growth", "honor the process", "trust the process", "lean into the discomfort", "the universe is asking you to…" are BANNED. End the interpretation on the specific real-life scene.
  (d) BANNED PHRASES (anywhere in any transit description, any reading type): "this part of your chart", "this part of your natal chart", "this part of your natal makeup", "this part of your makeup". If you catch one of these forming, rewrite the sentence to name the specific natal point.
  (e) WITHIN-ENTRY DEDUP: Do NOT write the same explanatory sentence twice inside a single transit window entry. If the same transit is referenced more than once in the same section, describe it ONCE in full and refer back with "this same Saturn window" / "this same Pluto window" — do NOT repeat the full interpretation.
  (f) ENFORCEMENT — NON-NEGOTIABLE FOR EVERY READING TYPE: This rule applies to natal, career, solar_return, money, health, spiritual, relocation, timing, general, AND every future reading type added later. Per-type sections MAY add stricter framing but MAY NOT relax this rule. Empty placeholder language ("this part of your chart" / "this part of your natal makeup" / "this transit hitting your chart") is a HARD FAIL — the entry will be rejected and the reading marked incomplete regardless of which reading type emitted it.
  (g) ANTI-RESTATEMENT RULE FOR TIMING WINDOWS — TWO-SENTENCE INFORMATION TEST: A timing window description must contain at most ONE sentence naming the transit + natal point and ONE sentence describing a concrete real-life scenario. The two sentences MUST add DIFFERENT information. The second sentence may NOT restate the first using different words. FORBIDDEN example: "Jupiter squaring your natal Sun at 28°11' Libra touches your identity, vitality, and sense of purpose. You may catch yourself overcommitting around your identity, vitality, and sense of purpose." — both sentences say the same thing. CORRECT example: "Jupiter squaring your natal Sun at 28°11' Libra in your 12th house. You may catch yourself saying yes to a high-visibility project on a Wednesday and by Friday realizing your calendar can't hold it without dropping your morning routine or one of your existing commitments." THE TEST: if you delete sentence 1, sentence 2 must still convey new, scene-level information about a moment that could happen in this person's actual week or month. If sentence 2 paraphrases sentence 1, rewrite sentence 2 as a concrete scene with people, decisions, or body experiences.

BASE RULE 2 — PRONOUN VOICE & THIRD-PERSON REWRITER COMPATIBILITY (applies to every sentence in every reading type):
  STRICT 2nd person ("you" / "your") throughout. NEVER use third-person pronouns ("they", "their", "them", "themselves", "he", "she", "his", "her", "hers", "himself", "herself") to refer to the subject of the reading. The only acceptable third-person references are to actual third parties the user mentioned (their boss, their partner, their child) — never to the subject themselves. Verb agreement MUST match second person: write "you were", "you are", "you have", "you do" — NEVER "you was", "you is", "you has", "you does". Before finalizing any sentence, scan it for third-person pronouns and rewrite in second person at the source. Writing in second person from the start prevents the downstream pronoun rewriter from having to correct grammar after the fact (which can introduce errors). This rule has NO exceptions in section openings, body paragraphs, bullets, summary items, or closing messages.

BASE RULE 3 — DEDUPLICATION (applies to every reading type):
  No paragraph, sentence, or near-duplicate sentence may appear more than once across the reading. If two sections genuinely need the same idea, name it ONCE in full and reference it briefly the second time with section-specific framing — never paste the same wording twice, and never paraphrase the same sentence with cosmetic word swaps. WITHIN a single section body, do not write two consecutive sentences that share most of their content tokens (e.g., "your drive runs into walls" twice with slightly different wording). Each natal aspect (e.g. "Mars square Saturn") may be discussed in AT MOST ONE narrative section per reading; pick the section where it lands hardest.

BASE RULE 4 — GROUND TRUTH CONSTRAINTS (applies to every reading type that uses placement tables):
  The placement tables provided in the chart context are the ONLY source of truth for planet positions, signs, degrees, houses, retrograde status, and lunar nodes. Before writing any sentence that references a placement, verify it against the correct table:
  - If both natal AND Solar Return placement tables are present, every natal claim must match the natal table EXACTLY and every SR claim must match the SR table EXACTLY. NEVER copy an SR sign/degree into a sentence about the natal version of that planet, and NEVER copy a natal sign/degree into a sentence about the SR version. This applies to every planet and to the lunar nodes.
  - NEVER write a sign for any node (natal North/South Node, SR North/South Node) that differs from the corresponding placement table.
  - 2-step pre-write check for every placement-naming sentence: (1) is this a natal claim or an SR claim? (2) does my sentence match the corresponding row in the correct table? If either check fails, rewrite.

BASE RULE 5 — RETROGRADE FLAG NORMALIZATION (applies to every reading type):
  Each planet's retrograde status is fixed by the placement table for the chart it belongs to. If a planet is marked retrograde in its table (with "R", "Rx", "℞", or "retrograde"), you MUST describe it as retrograde in every sentence that names it — never as direct, never with the retrograde marker dropped. Same rule in reverse: never invent a retrograde marker on a planet the table shows as direct. Natal retrograde status comes ONLY from the natal table. SR retrograde status comes ONLY from the SR table — they are independent.
  ENUMERATION-LIST SUB-RULE (NON-NEGOTIABLE — applies whenever you write a summary/at-a-glance list of planet placements, e.g. "SR planets at a glance", "Natal at a glance", "Key SR placements", or any bullet/inline list that enumerates multiple planets in the form "{chart} {Planet} in {Sign}" or "{chart} {Planet} in {Sign} (House N)"): for EVERY retrograde planet in that list (per its chart's placement table), you MUST include an inline retrograde marker — the literal " R" token between the planet name and the sign — exactly like "SR Saturn R in Pisces (House 4)" or "natal Mercury R in Gemini". The marker is required even though the same planet's retrograde status is already shown in the placement table above. Dropping the marker in an enumeration list is the same hard fail as dropping it in narrative prose. The marker format is the single uppercase letter R as a separate token (preferred), or "Rx", or "℞" — never silent omission. Direct planets get NO marker. This sub-rule applies to natal and SR enumerations alike, in every reading type.

BASE RULE 6 — VOICE RULES (applies to every reading type):
  - Speak directly to the person as if in a live reading, not writing an essay about them.
  ...
  - FORBIDDEN words anywhere in any reading: "wound", "metabolized", "metabolize", "archetypal", "archetype", "portal", "liminal". FORBIDDEN: "activation" (use "this transit" instead). FORBIDDEN: "calling" used as a noun.
  - Each narrative_section body: 200–400 words. Never exceed 500 (and the 2,000-character cap above always wins).

BASE RULE 7 — DEFAULT FOR UNRECOGNIZED READING TYPES (forward compatibility):
  Any new question_type added in the future inherits BASE RULES 1–10 AUTOMATICALLY, no exceptions. The base rules are NEVER opt-out and NEVER restate-required — they are always in force the moment a question_type is recognized OR defaulted. If a question_type is encountered that is NOT explicitly enumerated below (i.e., a new reading type added later), default to: (i) inherit ALL of BASE RULES 1–10 above without modification, (ii) use the structure of the closest analogous enumerated reading type (e.g., a new "education" type uses the "career" structure as its template; a new "family" type uses the "relationship" structure), (iii) follow the UNIVERSAL PROSE-BODY MANDATE, UNIVERSAL BULLET GROUNDING RULE, and UNIVERSAL SUMMARY_BOX TIMING CONSTRAINT exactly as written elsewhere in this prompt. Never silently drop the base rules just because the type is unfamiliar.

BASE RULE 8 — NATAL RETROGRADE ACKNOWLEDGMENT (applies to every reading type, every section that takes a retrograde natal planet as its focal subject):
  If a natal planet is marked retrograde in the "Natal Key Placements" table (with R, Rx, ℞, or "retrograde"), then in EVERY narrative_section that takes that planet as its focal subject (e.g., a Venus retrograde must be acknowledged in "How You Love" / "Your Earning Style" / "Love and Relationships This Year" / "Hidden Strengths" — wherever Venus is the focal planet of the section), you MUST acknowledge the retrograde status at least once in plain language — NOT just as a symbol or "Rx" tag after the planet name. Write a single sentence explaining what that natal retrograde means for THIS person specifically, grounded in the planet's sign, house, and aspects in this chart. Example for natal Venus retrograde: "Your Venus is retrograde, so love and value run on review for you — you revisit, second-guess, and need extra time before you trust what you actually want from a partner or what something is worth paying for." This rule applies to natal Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto, and Chiron retrograde alike — every reading type (natal, career, money, health, relationship, solar_return, spiritual, relocation, timing, general, and any future type), every focal section. The acknowledgment must be specific, not a generic stock sentence. SR retrograde status is independent and is acknowledged separately per BASE RULE 9.

BASE RULE 9 — UNIVERSAL SR RETROGRADE ACKNOWLEDGMENT (NON-NEGOTIABLE — applies to EVERY reading type that references Solar Return placements anywhere in the prose: solar_return, career, money, health, relationship, relocation, spiritual, timing, general, and any future type):
  For every SR planet marked retrograde in the "Solar Return Key Placements" table (R, Rx, ℞, or "retrograde"), the prose MUST acknowledge the retrograde status at least once in plain language in whichever section discusses that SR planet's life-domain for this reading type. A bare "(R)" tag after the planet name is NOT enough — you must write a single plain-English sentence (no jargon, no symbols) explaining what that SR retrograde means for THIS year framed in the reading's domain. Domain-specific framing examples:
  - SR Mercury retrograde, career reading: place in "Your Career Foundation", "Hidden Strengths", "11th House and Networking", or the Strategy Summary's "When to Act" / "What to Avoid" — e.g., "SR Mercury is retrograde this year, so communication, decisions, and contracts at work will run on review — expect to revisit conversations and rework agreements you thought were settled."
  - SR Mercury retrograde, money reading: place in the earning/spending/contracts section — e.g., "SR Mercury is retrograde this year, so purchases, contracts, and pricing decisions will need a second pass before you commit."
  - SR Mercury retrograde, health reading: place in the planning/treatment section — e.g., "SR Mercury is retrograde this year, so treatment plans and provider choices benefit from a second opinion before you commit."
  - SR Mercury retrograde, relationship reading: place in the communication section — e.g., "SR Mercury is retrograde this year, so important conversations may need to be revisited and clarified more than once before they actually land."
  - SR Mercury retrograde, relocation reading: place in the planning/decision section — e.g., "SR Mercury is retrograde this year, so the move logistics, contracts, and city research will benefit from a second pass before you commit."
  Same rule fires for SR Venus retrograde (values/relationships under review), SR Mars retrograde (timing of action under review), SR Jupiter retrograde (growth/expansion under review), SR Saturn retrograde (long-term commitments under review), and any outer-planet SR retrograde when those planets are referenced. Omitting the acknowledgment for any SR planet that is retrograde in the SR table is a HARD FAIL on every reading type, not just solar_return. The solar_return reading's per-type "SR RETROGRADE ACKNOWLEDGMENT" rule below restates this for emphasis but does NOT scope it down — every reading type that names an SR planet inherits BASE RULE 9.
...
- For question_type "relationship": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order — 12 sections total:
  ...
  4. narrative_section — "Natal Relationship Architecture" (PRIORITY ORDER IS MANDATORY — structure the narrative in this exact sequence:
   ...
   8. narrative_section — "Natal & Solar Return Overlay" (EXPLICIT cross-chart logic. MANDATORY: Include at least 3–5 explicit cross-chart activations when supported by the data. Do NOT summarize generally or give only 1–2 examples. For each overlay claim, name the SR factor, the natal factor, and the meaning. Required checks: SR Venus aspecting natal Venus/Moon/Mars/Juno/7th house ruler; SR ASC or DSC within 3° of natal Venus/Moon/Mars/Juno/DSC ruler/Saturn-if-7th-ruler; SR 5th ruler aspecting natal relationship indicators; SR 7th ruler aspecting natal relationship indicators; SR planets falling in natal 5th/7th/8th/11th house; SR angles activating natal Venus/Mars/Moon/7th ruler. Format each as: "SR [factor] [aspect] natal [factor] ([orb]°): [meaning]." Example: "SR Venus conjunct natal Mercury (2° orb): attraction is strongly tied to conversation, messaging, and mental connection this year." Example: "SR Saturn square natal Moon-Saturn (1° orb): this year pressure-tests emotional security and exposes where fear or self-protection shapes relationship choices." Synthesize what is triggered this year vs natal baseline.
      ...
      MANDATORY CLOSING PARAGRAPH — "The Year's Overarching Theme": After writing all individual cross-chart activations in this section's body, the SAME body string MUST end with one final closing paragraph. This closing paragraph: (1) begins on its own line with the bold heading **The Year's Overarching Theme**; (2) is exactly ONE paragraph of 3–5 sentences (not a list, not bullets); (3) synthesizes ALL the activations above into one cohesive story — what do they point toward together; (4) names the single most important thing this year is asking of this person in their relationships; (5) uses plain behavioral language (no jargon, no orbs, no aspect names if avoidable); (6) is decisive and direct — like a final takeaway, not a recap. The closing paragraph is part of the SAME "body" string of this narrative_section, appended to the end. It is NOT a separate section.)
   ...
   11. modality_element — "Natal Elemental & Modal Balance" (NON-NEGOTIABLE — this section MUST appear between the timing_section and the summary_box. It is the 11th section of 12. It uses type "modality_element" with the EXACT structure shown in the schema example: elements[], modalities[], polarity[], dominant_element, dominant_modality, dominant_polarity, balance_interpretation. Do NOT skip it. Do NOT merge it into another section. Do NOT replace it with a narrative_section. If you only output 11 sections instead of 12, the most common reason is that you skipped THIS section — do not skip it.)
    12. summary_box — "Relationship Strategy Summary" (MUST be decisive, direct, and slightly confrontational — like a friend who tells you the truth. Include EXACTLY these six items in this order and NO others — do not add "What This Year Is Best For", "What This Year Asks Of You", or any other thematic-summary item to this scaffold. Thematic year summaries belong in the prose body of earlier sections, NOT as a duplicate scaffold here:
       ...
       - "Right Now": Identify the 1–2 transits from the Timing Windows above whose exact peak falls within ±2 weeks of the current date and tell the person — in 2 to 4 sentences — what that specific transit is asking of them in this exact moment (this week / this month). Lead with the lived behavior or feeling, name the transit second. If no transit qualifies, write EXACTLY: "No major outer-planet transit is within two weeks of exact peak right now — this is a quieter moment to integrate what's already in motion rather than wait for a single window to open." Do NOT invent transits not in the Timing Windows above.
...
CONCRETE-SCENARIO RULE (MANDATORY — applies to EVERY narrative_section body, EVERY bullet text, EVERY timing interpretation, in EVERY reading type — relationship, career, health, money, relocation, spiritual, timing, general — no exceptions): Write every interpretation as if explaining it to a smart person who has never studied astrology. For each activation, give one concrete real-life scenario — something that could actually happen in a week or a month — BEFORE or INSTEAD OF the abstract principle. The scene comes first; the meaning is implied by the scene. FORBIDDEN WORDS — never appear anywhere in any reading: "wound", "metabolized", "metabolize", "activation", "portal", "archetypal", "archetype", "liminal". If one of these words is the only one that fits, the sentence is wrong — rewrite it as a scene. Do not stack more than two abstract ideas in a row without grounding them in a real example. SELF-CHECK: if you catch yourself writing three clauses that are all concepts with no scene, stop and rewrite. EXAMPLE OF WHAT NOT TO WRITE: "The wound around partnership is visible and asking to be looked at directly." EXAMPLE OF WHAT TO WRITE: "You may find yourself replaying an old argument or realizing mid-conversation that you've been accepting less than you actually want — that's this transit doing its job." This rule overrides any tendency toward poetic, abstract, or therapy-jargon phrasing — the test is always "could the reader name the moment from their own life this week?"
...
- Each of the 6 experience bullets should be 1-3 sentences of real-life description. No abstractions.
  - For question_type "relocation": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order — do NOT rearrange, combine, or skip sections between regenerations:
  ...
  3. narrative_section — "Astrocartography Lines" OR "Chart-Based Relocation Guidance" (METHODOLOGY-ONLY SECTION. If astrocartography line data is present, this section explains WHAT the lines are and reports the actual line/angle data with distances. If NOT present, label this section "Chart-Based Relocation Guidance" and explain the chart-derived reasoning method. Never fake line data.
  ...
  4b. narrative_section — "Your Location Choices" (CONDITIONAL — include this section ONLY when a "USER-SUPPLIED LOCATIONS" block appears later in the system prompt naming a current city or considering cities. Place this section AFTER "Location Fit Profiles" and BEFORE "Best Places Within [Country]". Follow the structure defined in the USER-SUPPLIED LOCATIONS block exactly: per user-supplied city, cover (1) Chart themes vs. this city, (2) What it offers that matches, (3) What it lacks or challenges, (4) Overall fit. The "body" field of this section is REQUIRED and must briefly introduce that this section analyzes the SPECIFIC cities the user named, distinct from the recommendations below. If no USER-SUPPLIED LOCATIONS block is present, OMIT this section entirely.)
  5. city_comparison — "Best Places Within [Country]" (3-5 recommended cities INSIDE the user's home country only, with full sub-scores, tags, supports, cautions, explanation. The bracket [Country] MUST be replaced with the actual country name in the title — e.g., "Best Places Within the United States", "Best Places Within the United Kingdom", "Best Places Within Canada", "Best Places Within Australia". HOME COUNTRY DETECTION: Detect the user's home country from (a) birth_info.location and (b) the USER-SUPPLIED LOCATIONS current city when present. If they conflict, prefer the current city. If undetectable, use the broader region/continent and state that explicitly in the section's first explanation. CITY POOL: Recommend ONLY realistic relocation options — major cities, regional hubs, university towns, well-known mid-size cities. Do NOT recommend obscure towns, tiny villages, or locations the average reader wouldn't recognize. Examples of valid pools: US → New York, Los Angeles, Chicago, Austin, Denver, Seattle, Boston, Atlanta, Portland, Nashville, Minneapolis, Asheville, Boulder, Santa Fe, Charleston, Burlington, Madison, Ann Arbor, etc. UK → London, Edinburgh, Bristol, Manchester, Brighton, Bath, Oxford, Cambridge, Glasgow, etc. The "explanation" field must focus on which natal/SR placements drive the score AND the lived daily-life experience for that city. Each city must include both supports AND cautions — no city gets a free pass.)
  6. narrative_section — "Your Location Choices" (CONDITIONAL — include ONLY when a USER-SUPPLIED LOCATIONS block appears later in the system prompt. Place this section AFTER "Best Places Within [Country]" and BEFORE "One International Option". Same per-city 4-step structure as defined in the USER-SUPPLIED LOCATIONS block. If no USER-SUPPLIED LOCATIONS block is present, OMIT this section entirely. NOTE: this section may also appear earlier as section 4b in the existing "Location Fit Profiles" flow; place it ONCE in the position that makes sense for the actual JSON ordering — do not duplicate.)
  7. narrative_section — "One International Option" (MANDATORY — a SINGLE city OUTSIDE the user's home country that the chart strongly supports. Frame this as "if you were ever open to living abroad" rather than a primary recommendation — this is a possibility, not a directive. Pick ONE city only — not a list, not a comparison. The "body" field must be 3-4 sentences total covering: (a) the city name and country, (b) which 1-2 chart placements/lines make it a strong fit, (c) what daily life there would feel like in plain language, (d) one honest tradeoff or limitation. Do NOT include bullets, do NOT include sub-scores, do NOT name multiple alternatives. Keep it brief and intriguing, not exhaustive. Opening phrase suggestions: "If you were ever open to living abroad...", "Outside your home country, the one place worth naming is...", "If international relocation ever enters the picture...".)
  8. timing_section — "Timing for a Move" (Transits to Moon, IC, 4th house ruler, 10th house ruler. Eclipses activating 4th/10th axis. Best move windows AND caution windows over next 12-18 months.)
  9. modality_element — "Natal Elemental & Modal Balance"
  10. summary_box — "Strategy Summary" with items: "Best Type of Place Overall", "What to Prioritize When Choosing", "What to Avoid Repeating (Pattern)", "Ideal Timing Window", "Analysis Mode". DO NOT add "Best Places Within [Country]" or any city-list item to the summary_box — those cities are already shown in the city_comparison card above and listing them again is pure repetition. The summary is for STRATEGIC PRINCIPLES the reader takes away, not a re-listing of cities. The body field still synthesizes the core takeaway in 2-3 sentences.
  ...
  RELOCATION CAUTION CITY CONTINUITY RULE (MANDATORY): Every city named anywhere in the reading — including the "cautions" field of any city card in "Best Places Within [Country]", the Strategy Summary "What to Avoid Repeating" item, or the "One International Option" tradeoff sentence — must have either already appeared earlier in the reading with context, OR include a one-sentence in-place note locating it for the reader (e.g., "Phoenix wasn't in the top recommendations because Solar Return Mars MC falls there, sharpening the urgency of its already-fast pace"). Never drop a brand-new city name with no prior context — the reader should never wonder "where did this city come from?"
...
- For question_type "career": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order:
  1. placement_table — "Key Placements"
  ...
  3. narrative_section — "Hidden Strengths" (6th house for daily work style, 2nd house for earning style, 8th house for joint ventures/investments).
     REQUIRED OUTPUT SHAPE — STRUCTURAL, NOT OPTIONAL: This section MUST be emitted with a non-empty 'body' (200–400 words of career prose grounded in the 6th/2nd/8th-house placements) AND with a 'bullets' array containing the following labeled bullets in this exact order. Each bullet's 'text' MUST be 1–3 non-empty sentences (no empty strings, no whitespace-only values, no placeholder text). Bullets with empty 'text' will be dropped by the cleanup pipeline and the section will fail review.
       (a) { "label": "Daily Work Superpower", "text": "<1–3 sentences naming the 6th-house sign, its ruler's placement, and any planets in the natal 6th — translated into the specific way THIS person performs day-to-day work (the daily-work edge their coworkers and clients actually notice). NEVER leave empty. NEVER reduce to a one-word answer. Example shape: 'Your 6th-house Aquarius (ruled by Uranus in the 8th) gives you a sharp eye for the broken systems other people learned to live with — you spot the inefficiency in a workflow on day two and quietly redesign it without making anyone feel called out.'" }
       (b) { "label": "Professional Tribe", "text": "<1–3 sentences naming the 11th-house sign, its ruler's placement, and any planets in the natal 11th — translated into the specific kind of professional community, network, and peer group THIS person actually belongs in (who their people are at work, what rooms they thrive in, what kind of collaboration brings out their best). NEVER leave empty. NEVER reduce to a one-word answer. Example shape: 'Your 11th-house Cancer pulls you into tight, loyalty-based professional circles — small teams that feel like family, mission-driven organizations, or long-running peer groups — rather than wide, transactional networking.'" }
       (c) { "label": "SR Mercury Retrograde Review", "text": "<one plain-English sentence — REQUIRED ONLY IF the 'Solar Return Key Placements' table marks SR Mercury retrograde (R, Rx, ℞, or 'retrograde'); OMIT this entire bullet entirely when SR Mercury is direct>. When required, the sentence MUST explicitly state that SR Mercury is retrograde this year AND explain what that means for communication, decisions, and contracts at work. Example: 'SR Mercury is retrograde this year, so communication, decisions, and contracts at work will run on review — expect to revisit conversations and rework agreements you thought were settled, and avoid signing or launching anything you have not had at least a second pass on.' This bullet is the structural slot where BASE RULE 9 / CAREER SR MERCURY RETROGRADE RULE land for career readings — emitting Hidden Strengths without this bullet when SR Mercury is retrograde in the SR table is a HARD FAIL." }
       (d) Same structural slot fires for SR Jupiter retrograde, SR Saturn retrograde, and SR Mars retrograde when those planets are marked retrograde in the SR table — add additional bullets labeled exactly "SR Jupiter Retrograde Review" / "SR Saturn Retrograde Review" / "SR Mars Retrograde Review" with the domain-framed explanation from the CAREER SR MERCURY RETROGRADE RULE. OMIT each retrograde-review bullet entirely when its planet is direct.
     The two career-content bullets (a) and (b) are ALWAYS required. They were dropped in a recent generation because their definitions were ambiguous; this strict schema restores them and prevents future drops.
  ...
  CAREER SR MERCURY RETROGRADE RULE (NON-NEGOTIABLE — explicit reinforcement of BASE RULE 9 for career readings): If the "Solar Return Key Placements" table marks SR Mercury retrograde (R, Rx, ℞, or "retrograde"), the career reading MUST include exactly one plain-English sentence stating that SR Mercury is retrograde this year and what that means for communication, decisions, and contracts in a work context. Place this sentence in EXACTLY ONE of: "Your Career Foundation", "Hidden Strengths", "11th House and Networking", or the Strategy Summary's "When to Act" / "What to Avoid" item. NEVER silently drop the acknowledgment because the SR Mercury position has already been mentioned in the placement table — the table is data, the prose must explain it. Example required sentence: "SR Mercury is retrograde this year, so communication, decisions, and contracts at work will run on review — expect to revisit conversations and rework agreements you thought were settled, and avoid signing or launching anything you have not had at least a second pass on." The same explicit-acknowledgment rule fires for SR Saturn retrograde (long-term commitments under review — restructure existing structures rather than build new ones), SR Jupiter retrograde (growth and expansion under review — internal cultivation rather than visible launch), and SR Mars retrograde (timing of action under review — finish unfinished business before starting new initiatives) when those planets are marked retrograde in the SR table. Omitting any required SR retrograde acknowledgment in a career reading is a HARD FAIL.
  ...
  CAREER TRANSIT SPECIFICITY RULE (MANDATORY for every entry in "Career Timing Windows" AND any inline transit reference in any career narrative_section AND any "When" / "Best Windows" / "Caution Windows" item in the Strategy Summary): Every transit interpretation MUST (1) name the specific natal point being hit with sign and degree and (when available) house — write "Saturn squaring your natal Sun at 28°11' Libra in your 10th house", NEVER "Saturn squaring this part of your chart", NEVER "this part of your natal makeup", NEVER "this transit hitting your chart"; (2) give ONE concrete career-specific real-life scenario specific to what THAT natal point means for THIS person — a workplace conversation, a contract decision, a visibility moment, a calendar choice, a money-at-work decision, a moment of being seen or overlooked — NOT a generic textbook description of what the transit does in general; (3) NEVER end with a generic horoscope closer — "awareness is the work", "the lesson is to stay present", "trust the process", "lean into the discomfort" are BANNED. End on the specific real-life career scene. The phrases "this part of your chart", "this part of your natal chart", and "this part of your natal makeup" are BANNED in every career transit description, no exceptions — they are a HARD FAIL and the entry will be rejected. The two-sentence anti-restatement test from BASE RULE 1(g) applies: sentence 2 must add new scene-level information, not paraphrase sentence 1. If the same transit is referenced more than once in the section, describe it ONCE in full and refer back with "this same Saturn window" — do NOT repeat the full interpretation.
...
- For question_type "health": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order:
  1. placement_table — "Key Placements"
  ...
  HEALTH CORE STRENGTH RULE (MANDATORY — NON-NEGOTIABLE): The "Core Strength" item in the Health Strategy Summary MUST describe the person's genuine, durable NATAL health strengths — synthesized from constitution and chart fundamentals such as: dominant element (e.g., water dominant = strong emotional-somatic feedback, intuitive body wisdom), Ascendant sign (e.g., Virgo Ascendant = early-warning system, sensitive to small imbalances), Chiron sign/house (especially Chiron in the 6th = healing gift / wounded-healer in daily routines), benefic placements in the 6th house (e.g., a Jupiter–Mercury conjunction in the 6th = intellectual resilience and mental engagement with health), well-aspected luminaries, strong Mars, etc. ABSOLUTELY FORBIDDEN in "Core Strength": transit names, planet+aspect+date references, month names, date ranges, the word "windows", "this year", or any timing language whatsoever. Timing windows belong EXCLUSIVELY in the "Timing" item (and any "Best Windows" / "Caution Windows" / "Extra Care Windows" / "Restorative Windows" items if present). "Core Strength" and "Timing" / "Best Windows" MUST NOT contain duplicate or overlapping content — if "Core Strength" reads like a timing recap, rewrite it from natal constitution before emitting the JSON.
  ...
  HEALTH SR MERCURY RETROGRADE RULE (NON-NEGOTIABLE — explicit reinforcement of BASE RULE 9 for health readings): If the "Solar Return Key Placements" table marks SR Mercury retrograde, the health reading MUST include exactly one plain-English sentence explaining what that means for treatment plans, provider choices, scheduling, and second opinions this year. Place it in "Healing & Recovery", "Stress Points & Vulnerabilities", or the Strategy Summary's "Best Practices" / "Watch Points" item — never silently omit it. Same rule fires for SR Saturn retrograde (long-term protocols and structural commitments under review), SR Mars retrograde (timing of physical effort under review — finish recovery before pushing intensity), and SR Jupiter retrograde (scope and ambition of the wellness plan under review) when those planets are marked retrograde in the SR table. Omitting any required SR retrograde acknowledgment is a HARD FAIL.

- For question_type "money": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order:
  1. placement_table — "Key Placements"
  ...
  MONEY SR MERCURY RETROGRADE RULE (NON-NEGOTIABLE — explicit reinforcement of BASE RULE 9 for money readings): If the "Solar Return Key Placements" table marks SR Mercury retrograde, the money reading MUST include exactly one plain-English sentence explaining what that means for purchases, contracts, pricing decisions, and account changes this year. Place it in "Your Earning Style", "Shared Resources & Investments", or the Strategy Summary's "When to Act" / "What to Avoid" item — never silently omit it. Same rule fires for SR Venus retrograde (values and what feels worth paying for under review), SR Jupiter retrograde (expansion bets and risk appetite under review), and SR Saturn retrograde (long-term financial structures under review) when those planets are marked retrograde in the SR table. Omitting any required SR retrograde acknowledgment is a HARD FAIL.
- For question_type "natal": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type.
  ...
  - TIMING TRANSIT SPECIFICITY (MANDATORY for "Where You Are in Your Life Cycles" and any inline transit references in any natal narrative_section): Every transit interpretation MUST (1) name the specific natal point being hit with sign and house — write "Saturn squaring your natal Sun in Leo in your 2nd house", NEVER "Saturn squaring this part of your natal chart", NEVER "this part of your natal makeup", NEVER "this transit hitting your chart"; (2) give one concrete real-life scenario specific to what THAT natal point means for THIS person — a situation that could plausibly happen in a week or a month, NOT a generic textbook description of what the transit does in general; (3) NEVER end with a generic closer — the phrases "awareness is the work", "the lesson is to stay present", "trust the process", "lean into the discomfort" are BANNED. End on the specific real-life scene. The phrases "this part of your natal chart" and "this part of your natal makeup" are BANNED in every natal transit description, no exceptions. If the same transit is referenced more than once in the same section, describe it ONCE in full and refer back with "this same Saturn window" — do NOT repeat the full interpretation paragraph.

- For question_type "solar_return": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. You have access to pre-calculated Solar Return data injected into this prompt under the heading "--- SOLAR RETURN ANALYSIS (PRE-CALCULATED — PRIMARY SOURCE OF TRUTH) ---". Use it as your primary source of truth. Do not contradict it. The profection year, Time Lord, moon phase, stelliums, and SR-to-natal aspects are already calculated — build your prose from them rather than re-deriving. The natal portrait data gives you the permanent baseline; the SR data tells you what this specific year is doing to that baseline.

  SECTION 8 ("The SR House Emphasis") IS MANDATORY — HARD FAIL IF MISSING. If this section is absent from the output, the reading is incomplete and will be rejected. Do not skip this section under any circumstance, even if no SR house contains 2+ planets — in that case, follow the fallback instructions inside the section spec (name the strongest single SR planet's house and treat that as the year's emphasis).
  ...
  - NEVER write an SR placement (sign, degree, house, retrograde) that differs from the "Solar Return Key Placements" table. Specifically: if the SR table marks SR Saturn as retrograde (Rx, ℞, R, or "retrograde"), you MUST describe SR Saturn as retrograde — never as direct, never omit the retrograde status. Same rule for every SR planet marked retrograde.
  ...
  - TIMING TRANSIT SPECIFICITY (MANDATORY for every transit interpretation in the solar_return "Solar Return Timing Windows" section AND for any inline transit references in narrative_sections): Every transit interpretation MUST (1) name the specific natal point being hit with sign and house — write "Pluto squaring your natal Mars in Scorpio in your 5th house", NEVER "Pluto squaring this part of your chart", NEVER "this part of your natal makeup", NEVER "this transit hitting your chart"; (2) give one concrete real-life scenario specific to what THAT natal point means for THIS person — a situation that could plausibly happen in a week or a month, NOT a generic textbook description; (3) NEVER end with a generic closer — the phrases "awareness is the work", "the lesson is to stay present", "trust the process", "lean into the discomfort" are BANNED. End on the specific real-life scene. The phrases "this part of your natal chart" and "this part of your natal makeup" are BANNED in every solar_return transit description, no exceptions. This applies equally to every entry in the Solar Return Timing Windows section AND to every transit reference in the SR Strategy Summary "When" / "Best Windows" / "Caution Windows" callouts. Empty placeholder language ("this part of your chart" / "this part of your natal makeup" / "this transit hitting your chart") is a HARD FAIL — the entry will be rejected. The two-sentence anti-restatement test from BASE RULE 1(g) applies: sentence 2 must add new scene-level information, not paraphrase sentence 1.

- For question_type "spiritual": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use this EXACT section order:
  1. placement_table — "Key Placements"
...
- For question_type "timing": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Lead with timing_section, then narrative_section, then modality_element, then summary_box.
- For question_type "general": INHERITS UNIVERSAL BASE — BASE RULES 1–10 are non-overridable for this reading type. Use narrative_section sections only + modality_element + summary_box.
      ...
      if (parsedContent && typeof parsedContent === "object" && !Array.isArray(parsedContent)) {
        ...
        if (parsedContent.sections && Array.isArray(parsedContent.sections)) {
          ...
          const requiredByType: Record<string, { type: string; titleHint: string }[]> = {
            ...
            natal: [
              { type: 'placement_table', titleHint: 'Natal Key Placements' },
              { type: 'narrative_section', titleHint: 'Who You Are' },
              { type: 'narrative_section', titleHint: 'How You Think' },
              { type: 'narrative_section', titleHint: 'How You Love' },
              { type: 'narrative_section', titleHint: 'How You Act' },
              { type: 'narrative_section', titleHint: 'Where You Grow' },
              { type: 'narrative_section', titleHint: 'Where You Are Being Tested' },
              { type: 'narrative_section', titleHint: 'Soul Purpose' },
              { type: 'narrative_section', titleHint: 'The Pattern and the Gift' },
              { type: 'narrative_section', titleHint: 'Generational Placement' },
              { type: 'narrative_section', titleHint: 'Patterns and Themes' },
              { type: 'narrative_section', titleHint: 'Life Cycles' },
              { type: 'narrative_section', titleHint: 'Overarching Message' },
              { type: 'modality_element', titleHint: 'Balance' },
              { type: 'summary_box', titleHint: 'Strategy' },
            ],
            solar_return: [
              { type: 'placement_table', titleHint: 'Natal Key Placements' },
              { type: 'placement_table', titleHint: 'Solar Return Key Placements' },
              { type: 'narrative_section', titleHint: "Year's Central Theme" },
              { type: 'narrative_section', titleHint: 'Emotional Tone' },
              { type: 'narrative_section', titleHint: 'Communication and Thinking' },
              { type: 'narrative_section', titleHint: 'Love and Relationships' },
              { type: 'narrative_section', titleHint: 'Energy, Drive' },
              { type: 'narrative_section', titleHint: 'Where Growth and Luck' },
              { type: 'narrative_section', titleHint: 'Pressure and Discipline' },
              { type: 'narrative_section', titleHint: 'SR House Emphasis' },
              { type: 'narrative_section', titleHint: 'SR-to-Natal' },
              { type: 'timing_section', titleHint: 'Solar Return Timing' },
              { type: 'narrative_section', titleHint: 'Left Behind' },
              { type: 'narrative_section', titleHint: 'Single Most Important Message' },
              { type: 'modality_element', titleHint: 'Balance' },
              { type: 'summary_box', titleHint: 'Strategy' },
            ],
          };

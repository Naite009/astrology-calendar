...
BASE RULE 7 — DEFAULT FOR UNRECOGNIZED READING TYPES (forward compatibility):
  Any new question_type added in the future inherits BASE RULES 1–9 AUTOMATICALLY, no exceptions. The base rules are NEVER opt-out and NEVER restate-required — they are always in force the moment a question_type is recognized OR defaulted. If a question_type is encountered that is NOT explicitly enumerated below (i.e., a new reading type added later), default to: (i) inherit ALL of BASE RULES 1–9 above without modification, (ii) use the structure of the closest analogous enumerated reading type (e.g., a new "education" type uses the "career" structure as its template; a new "family" type uses the "relationship" structure), (iii) follow the UNIVERSAL PROSE-BODY MANDATE, UNIVERSAL BULLET GROUNDING RULE, and UNIVERSAL SUMMARY_BOX TIMING CONSTRAINT exactly as written elsewhere in this prompt. Never silently drop the base rules just because the type is unfamiliar.

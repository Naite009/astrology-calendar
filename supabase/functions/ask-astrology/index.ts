...
BASE RULE 1 — TIMING TRANSIT PERSONALIZATION (applies to every transit interpretation, in every reading type, in every section that names a transit — timing_section, narrative_section inline references, summary_box "When" / "Best Windows" / "Caution Windows" callouts):
  ...
  (b) GIVE ONE CONCRETE REAL-LIFE SCENARIO specific to what THAT natal point means for THIS person — a situation that could plausibly happen in a week or a month. Do NOT write a generic textbook description (e.g., "brings intensity and power dynamics to the surface"). The scenario must be grounded in what the natal point actually rules in this chart.

...
JSON SCHEMA (return exactly this shape, NEW SECTION STRUCTURE):
{
  ...
  "parentChildConnections": [
    { "parent": "ParentName", "child": "ChildName", "body": string (3-5 sentences. REQUIRED for EVERY parent-child pair in the family — no skipping, even if the pair is complex, friction-heavy, or sparse on aspects. Describe the dynamic honestly: open by naming the pair, then cite the strongest named synastry aspects between them (bridges AND friction). Include BOTH (a) what can work in this specific pair, AND (b) what can feel difficult — apply the ASPECT REALITY RULE so "what works" is stated as a range, not a guarantee. If contacts are sparse, say so plainly in one sentence rather than invent. Plain English, observable behavior, inline citation of the aspect(s) used. NO therapy phrasing. NO symbolic astrology.) }
  ],
  "childAdaptations": [

...
description: Layer 3 "What This Child Needs From You" — 3 mechanism-mapped recognition lines per child, gated on a valid childMechanism with internal conflict + cause→effect; repairs invalid mechanisms before response
type: feature
---
...
If invalid → the edge function must run a repair pass that strengthens the child mechanism first, then regenerates the needs lines. If repair still fails, return a validation error. NEVER surface null/generic user-facing content and NEVER fall back to "be patient", "give space", etc.
...
If fewer than 3 lines survive scrubbing → log `needs_section_underfilled` and trigger repair. Gate failures log `needs_section_blocked_weak_mechanism` (pair) or `needs_blocked_weak_mechanism:<childName>` (system) on `payload._validation_log`. Final response may not include failed needs entries.
